...
data_dir = "/project/mgreenst/energy_flux"
...
def process_file(cp):
    ...
    if len(cp_geo_list) == 1:
        ...
        print(f"Error: MOD03 file for file {cp} doesn't exist.", flush=True)
        return None, None
    ...
    if (qa5_a["_FillValue"] == 0) & (qa5_a['add_offset'] == 0) & (qa5_a['scale_factor'] == 1):
        ...
        print("FillValue != 0 or add_offset != 0 or scale_factor!= 1. Error." , flush=True)
        sys.exit()
    ...
    del lat, lon, xs, ys
    del qa1_dict, qa1_hdf
    _ = gc.collect()

    return data_1km, data_5km
...
if __name__ == "__main__":
    ...
    for day in range(1,101):
        print(f"{year} {day}")
        cloud_paths = sorted(glob.glob(f"{data_dir}/modis_l2/{cloud_file}/{year}/{str(day).zfill(3)}/{cloud_file}.A{year}{str(day).zfill(3)}.*.hdf"))
        ...
        for data_1km, data_5km in results:
            if data_1km is not None:
                data_1km_collector.append(data_1km)
            if data_5km is not None:
                data_5km_collector.append(data_5km)

        del results
        _ = gc.collect()

    pd.concat(data_1km_collector).to_csv(f"{data_dir}/intermediate/modis/1km_2020_1-100.csv")
    pd.concat(data_5km_collector).to_csv(f"{data_dir}/intermediate/modis/5km_2020_1-100.csv")