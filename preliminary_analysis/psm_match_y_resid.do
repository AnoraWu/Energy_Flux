...
foreach var in sw_toa_flux_up net_sw_surface_flux sw_radiance_up {
	...
	scatter `var' solar_zenith, title("relationship of zenith and `vlabel'") xtitle("solar zenith") ytitle("`vlabel'")
	graph export "$output/zenith_`var'_no0.png", as(png) replace
}
restore
...
* commented this part, because we already done this in processing the data
// replace sw_toa_flux_up = . if sw_toa_flux_up == 0
// replace sw_radiance_up = . if sw_radiance_up == 0
// replace net_sw_surface_flux = . if sw_toa_flux_up == 0
...
* interpolate for missing days if missing days <= 10 days
bys dt_adcode (date): ipolate solar_zenith date, gen(solar_zenith_ipol)
drop solar_zenith
ren solar_zenith_ipol solar_zenith
...
// 	gen `var'_mark = (`var'_ipol < . & `var'>=.)
	
	* regress and construct residuals
	reg `var' solar_zenith
	predict `var'_resid, resid
	
	local vlabel : variable label `var'
	label variable `var'_resid "`vlabel' residual"
	drop `var' //`var'_ipol miss gapid gapcount maxgap  
	
}

save "$raw_data/skeleton_merged2024_ssf_resid.dta", replace

local varlist net_sw_surface_flux_resid  sw_toa_flux_up_resid sw_radiance_up_resid
...
forval i = 1/7 {
	gen sw_toa_flux_up_resid_`i' = round(l`i'.sw_toa_flux_up_resid, 20)
}

forval i = 1/7 {
	gen net_sw_surface_flux_resid_`i' = round(l`i'.net_sw_surface_flux_resid, 30)
}

forval i = 1/7 {
	gen sw_radiance_up_resid_`i' = round(l`i'.sw_radiance_up_resid, 10)
}
